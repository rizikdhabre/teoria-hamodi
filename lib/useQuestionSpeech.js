...
import { isRealTtsEnabled } from './ttsEnvironment.mjs';

const REAL_TTS_ENABLED = isRealTtsEnabled(process.env.NODE_ENV);
...
export function useQuestionSpeech(lang) {
  ...
  const preload = useCallback(async (items) => {
    if (!REAL_TTS_ENABLED || typeof window === 'undefined') return;
  ...
  const speak = useCallback(async ({
    ...
    try {
      if (!REAL_TTS_ENABLED || !docId || typeof window === 'undefined') return;
  ...
  }, [getAudioUrl, lang, preloadQuestionBundle, showPendingMessage]);
  ...
  return { preload, speak, stop, isSpeaking, statusMessage };
}
