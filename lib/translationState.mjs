...
export function createTranslationState() {
  ...
  function register(id, sources) {
    const before = new Set(activeSources(registrations));
    ...
    if (failed) {
      for (const source of activeSources(registrations)) {
        if (!before.has(source)) failed.delete(source);
      }
    }
    return true;
  }
  ...
  function createRequest() {
    ...
    const sources = activeSources(registrations).filter(
